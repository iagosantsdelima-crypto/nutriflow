...
NIVEIS = [
    ...
    {"nome": "Focado",     "emoji": "🎯", "min_pts": 100},
    {"nome": "Dedicado",   "emoji": "💪", "min_pts": 300},
    {"nome": "Atleta",     "emoji": "🏃", "min_pts": 700},
    {"nome": "Monstro",    "emoji": "🔥", "min_pts": 1500},
    {"nome": "Lenda",      "emoji": "👑", "min_pts": 3000},
]

CONQUISTAS = [
    {"id": "primeiro_registro", "emoji": "🌟", "nome": "Primeira Refeicao",  "desc": "Registrou sua primeira refeicao",          "pts": 10,  "check": lambda ud: any(ud["alimentacao"].values())},
    {"id": "meta_dia",          "emoji": "🎯", "nome": "Meta Batida",         "desc": "Atingiu a meta calorica pela primeira vez", "pts": 25,  "check": lambda ud: any(sum(r["calorias"] for r in refs) >= ud.get("meta_calorias", 2000) * 0.95 for refs in ud["alimentacao"].values() if refs)},
    {"id": "streak_3",          "emoji": "🔥", "nome": "Em Chamas",           "desc": "3 dias seguidos de treino",                 "pts": 30,  "check": lambda ud: _streak(ud) >= 3},
    {"id": "streak_7",          "emoji": "💥", "nome": "Semana Perfeita",     "desc": "7 dias seguidos de treino",                 "pts": 75,  "check": lambda ud: _streak(ud) >= 7},
    {"id": "streak_30",         "emoji": "🏆", "nome": "Maquina",             "desc": "30 dias seguidos de treino",                "pts": 300, "check": lambda ud: _streak(ud) >= 30},
    {"id": "treino_10",         "emoji": "💪", "nome": "Dedicado",            "desc": "10 treinos concluidos",                     "pts": 50,  "check": lambda ud: sum(1 for t in ud["treinos"].values() if t.get("concluido")) >= 10},
    {"id": "treino_50",         "emoji": "🦾", "nome": "Atleta",              "desc": "50 treinos concluidos",                     "pts": 200, "check": lambda ud: sum(1 for t in ud["treinos"].values() if t.get("concluido")) >= 50},
    {"id": "log_7",             "emoji": "📒", "nome": "Consistente",         "desc": "7 dias com alimentacao registrada",         "pts": 50,  "check": lambda ud: len([d for d, v in ud["alimentacao"].items() if v]) >= 7},
    {"id": "log_30",            "emoji": "📅", "nome": "Habito Formado",      "desc": "30 dias com alimentacao registrada",        "pts": 200, "check": lambda ud: len([d for d, v in ud["alimentacao"].items() if v]) >= 30},
    {"id": "kcal_1000",         "emoji": "⚡", "nome": "Mil Calorias",        "desc": "Registrou 1000 kcal em um dia",             "pts": 15,  "check": lambda ud: any(sum(r["calorias"] for r in refs) >= 1000 for refs in ud["alimentacao"].values() if refs)},
]
...
def gerar_frase_inteligente(kcal_hoje, meta, macros, hora_atual=None):
    """Gera mensagem contextual e inteligente baseada no progresso"""
    pct = kcal_hoje / meta * 100 if meta > 0 else 0
    restante = meta - kcal_hoje
    prot = macros.get("proteina", 0)
    meta_prot = 140  # ref aproximada

    if pct == 0:
        return "Vamos comecar! Registre sua primeira refeicao 💪", "#22d3ee"
    elif pct < 15:
        return f"Bom inicio — {pct:.0f}% da meta. Continue registrando!", "#22d3ee"
    elif pct < 40:
        if prot < 30:
            return f"Faltam {restante:.0f} kcal. Dica: adicione proteina no proximo registro 🥩", "#22d3ee"
        return f"Voce esta em {pct:.0f}% — bom comeco de dia!", "#22d3ee"
    elif pct < 70:
        return f"Indo bem! Se continuar assim, bate a meta hoje 🎯", "#34d399"
    elif pct < 85:
        if prot < meta_prot * 0.5:
            return f"Quase la! Faltam {restante:.0f} kcal. Que tal proteina no proximo?", "#fbbf24"
        return f"Excelente progresso — {pct:.0f}% concluido!", "#34d399"
    elif pct < 95:
        return f"Quase la! Mais {restante:.0f} kcal para bater a meta ✨", "#fbbf24"
    elif pct <= 108:
        return "Meta batida! Dia perfeito 🎉", "#34d399"
    else:
        exc = kcal_hoje - meta
        return f"Atencao: {exc:.0f} kcal acima da meta. Tudo bem eventualmente!", "#fb7185"
...
def gerar_insights(udata, meta: int) -> list:
    ...
    insights.append({"emoji": "📊", "txt": f"Media nos ultimos {len(dias_com_dados)} dias: {media:.0f} kcal/dia"})
    ...
    if len(kcals_list) >= 5:
        ...
        if recente < anterior * 0.92:
            insights.append({"emoji": "📉", "txt": f"Tendencia de queda: ultimos 3 dias {anterior - recente:.0f} kcal abaixo da media anterior"})
        elif recente > anterior * 1.08:
            insights.append({"emoji": "📈", "txt": f"Consumo crescendo: ultimos 3 dias {recente - anterior:.0f} kcal acima do periodo anterior"})
...
# ══════════════════════════════════════════════════════════════════════════════
# CSS — REGRA FUNDAMENTAL: st.markdown com HTML NUNCA mistura com widgets
# Todos os st.button ficam fora de blocos HTML. Zero divs abertas sem fechar.
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
.prog-phrase { font-family: 'Syne', sans-serif; font-size: 1rem; font-weight: 700; }
...
/* ── Food rows — REGRA: div completo, autônomo, sem widgets dentro ── */
.food-row {
...
.food-row-qty  { font-size: 0.73rem; color: #334155; margin-top: 0.1rem; }
.food-row-kcal { font-family: 'Syne', sans-serif; font-size: 0.9rem; font-weight: 700; color: #22d3ee; white-space: nowrap; }

/* ── Meal section header — sem widgets dentro ── */
.meal-section-label {
  ...
  border-bottom: 1px solid #1e2a3a; margin-bottom: 0.4rem;
}
.meal-section-kcal {
  font-family: 'Syne', sans-serif; font-size: 0.78rem; font-weight: 700; color: #22d3ee;
}
...
.conquista-locked {
  background: #0f1520; border: 1px solid #1e2a3a; border-radius: 12px;
  padding: 0.9rem 1rem; display: flex; gap: 0.7rem; align-items: center;
...
.nivel-badge {
  ...
  background: linear-gradient(135deg, #1a0c2a, #2a1a3a);
  border: 1px solid #818cf855; border-radius: 999px;
  padding: 0.28rem 0.8rem;
  font-family: 'Syne', sans-serif; font-size: 0.78rem; font-weight: 700; color: #c4b5fd;
}
...
.feed-item {
  ...
  background: #0f1520; border-radius: 11px; border: 1px solid #1e2a3a;
...
.feed-av {
  ...
  color: #080b10; flex-shrink: 0;
}

.react-btn {
  display: inline-flex; align-items: center; gap: 0.2rem;
  background: #131c2a; border: 1px solid #1e2a3a; border-radius: 999px;
  padding: 0.18rem 0.55rem; font-size: 0.75rem; color: #475569; cursor: pointer;
}
...
.obj-prog-wrap { margin: 0.5rem 0 1rem; }
.obj-prog-bg   { background: #1e2a3a; border-radius: 999px; height: 8px; overflow: hidden; }
.obj-prog-fill { height: 100%; border-radius: 999px; transition: width 0.6s ease; }
</style>
...
<div style="text-align:center;padding:2.5rem 0 1rem">
  <div style="font-family:Syne,sans-serif;font-size:3rem;font-weight:800;background:linear-gradient(135deg,#22d3ee,#818cf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">
    ⚡ NutriFlow
  </div>
...
""", unsafe_allow_html=True)
    ...
    if pg == "login":
        ...
        with col:
            ...
            st.markdown(
                '<div style="background:#080b10;border-radius:12px;padding:1rem;text-align:center;'
                'margin:0.8rem 0;border:1px solid #1e2a3a">'
                '<div style="font-size:0.7rem;color:#475569;text-transform:uppercase;letter-spacing:.07em">Meta calorica estimada</div>'
                f'<div style="font-family:Syne,sans-serif;font-size:2rem;font-weight:800;color:{cor_obj_map.get(objetivo,"#22d3ee")}">{mv} kcal</div>'
                f'<div style="font-size:0.75rem;color:#334155">{objetivo}</div>'
                '</div>',
                unsafe_allow_html=True
            )
...
if novas_c:
    save()
    for c in novas_c:
        st.toast(f"{c['emoji']} Conquista desbloqueada: {c['nome']}! +{c['pts']} pts", icon="🏆")
...
with ch1:
    ...
    nivel_atual, nivel_prox = calcular_nivel(pts_user)
    str_html  = f'<span class="streak-badge">🔥 {streak} dias</span>' if streak >= 2 else ""
    niv_html  = f'<span class="nivel-badge">{nivel_atual["emoji"]} {nivel_atual["nome"]}</span>'
    st.markdown(
        '<div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;margin-top:0.3rem">'
        f'<span style="color:#334155;font-size:0.82rem">Ola, <strong style="color:#64748b">{udata["nome"]}</strong></span>'
...
if st.session_state.mostrar_dica:
    ...
    with dc1:
        st.markdown(
            '<div class="dica-box">'
            f'<div class="dica-ico">{ico_d}</div>'
            f'<div><div class="dica-ttl">{ttl_d}</div><div class="dica-txt">{d_raw[1]}</div></div>'
            '</div>',
            unsafe_allow_html=True
        )
    with dc2:
...
frase, cor_p = gerar_frase_inteligente(kcal_hoje, META, macros_hoje)
pct_c  = min(pct_meta, 100)
cor_mv = "#22d3ee" if pct_meta < 75 else ("#34d399" if pct_meta <= 108 else "#fb7185")

m1, m2, m3, m4 = st.columns(4)
with m1:
    st.markdown(
        '<div class="metric-box">'
        f'<div class="metric-value" style="color:{cor_mv}">{kcal_hoje:.0f}</div>'
        '<div class="metric-label">kcal hoje</div>'
        f'<div class="metric-sub">{pct_meta:.0f}% da meta</div>'
        '</div>',
        unsafe_allow_html=True
    )
with m2:
    st.markdown(
        '<div class="metric-box">'
        f'<div class="metric-value" style="color:#818cf8">{META}</div>'
        '<div class="metric-label">meta calorica</div>'
        f'<div class="metric-sub">{restante:.0f} restam</div>'
        '</div>',
        unsafe_allow_html=True
    )
with m3:
    st.markdown(
        '<div class="metric-box">'
        f'<div class="metric-value" style="color:#22d3ee">{media_sem:.0f}</div>'
...
with m4:
    ...
    st.markdown(
        '<div class="metric-box">'
        f'<div class="metric-value" style="color:{cor_s}">{streak}🔥</div>'
        '<div class="metric-label">streak treinos</div>'
        f'<div class="metric-sub">{treinos_7}/7 essa semana</div>'
        '</div>',
        unsafe_allow_html=True
    )

# Barra de progresso com frase inteligente
st.markdown(
    '<div class="prog-wrap">'
    '<div class="prog-label">'
    f'<span class="prog-phrase" style="color:{cor_p}">{frase}</span>'
    f'<span class="prog-pct">{kcal_hoje:.0f} / {META} kcal</span>'
    '</div>'
    '<div class="prog-bg">'
    f'<div class="prog-fill" style="width:{pct_c:.1f}%;background:linear-gradient(90deg,{cor_p},{cor_p}99)"></div>'
    '</div>'
    '</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    ...
    f'<div class="macro-label-row"><span>🥩 Proteina</span><span style="color:#22d3ee">{macros_hoje["proteina"]:.0f}g / {meta_prot:.0f}g</span></div>'
    f'<div class="macro-bg"><div class="macro-fill" style="width:{p_prot:.0f}%;background:#22d3ee"></div></div>'
    '</div>'
    '<div style="flex:1;min-width:120px">'
    f'<div class="macro-label-row"><span>🍚 Carboidrato</span><span style="color:#fbbf24">{macros_hoje["carbo"]:.0f}g / {meta_carbo:.0f}g</span></div>'
    f'<div class="macro-bg"><div class="macro-fill" style="width:{p_carb:.0f}%;background:#fbbf24"></div></div>'
    '</div>'
    '<div style="flex:1;min-width:120px">'
    f'<div class="macro-label-row"><span>🥑 Gordura</span><span style="color:#818cf8">{macros_hoje["gordura"]:.0f}g / {meta_gord:.0f}g</span></div>'
    f'<div class="macro-bg"><div class="macro-fill" style="width:{p_gord:.0f}%;background:#818cf8"></div></div>'
    '</div>'
    '</div>',
    unsafe_allow_html=True
)
...
with tab1:
    ...
    with col_add:
        ...
        if ontem_ref and not udata["alimentacao"].get(selected_str):
            ...
            st.markdown(
                '<div style="background:#0a1e0a;border:1px solid #166534;border-radius:12px;'
                ...
                f'<div style="font-size:0.75rem;color:#475569">{len(ontem_ref)} itens · {kcal_ont:.0f} kcal</div>'
                '</div>',
                unsafe_allow_html=True
            )
        ...
        # ── Modo Rápido — clique 1x nos frequentes já adiciona ───────────
        sugestoes = sugerir_almoco(udata)
        if sugestoes:
            st.markdown('<div style="font-size:0.78rem;color:#334155;margin-bottom:0.4rem">⚡ Adicionar rapido (1 clique):</div>', unsafe_allow_html=True)
            tipo_rapido = TIPOS_REFEICAO[0]
            # Determina tipo de refeição pelo horário
            hora = date.today().timetuple().tm_hour if selected_str == today_str else 12
            if hora < 10:   tipo_rapido = TIPOS_REFEICAO[0]
            elif hora < 12: tipo_rapido = TIPOS_REFEICAO[1]
            elif hora < 15: tipo_rapido = TIPOS_REFEICAO[2]
            elif hora < 18: tipo_rapido = TIPOS_REFEICAO[3]
            elif hora < 21: tipo_rapido = TIPOS_REFEICAO[4]
            else:           tipo_rapido = TIPOS_REFEICAO[5]

            for s in sugestoes:
                if s not in BANCO_ALIMENTOS:
                    continue
                info = BANCO_ALIMENTOS[s]
                qtd_pad = 100.0
                kcal_pad = kcal_estimada(s, qtd_pad, info["unidade"])
                mac_pad  = macros_estimados(s, qtd_pad, info["unidade"])
                col_chip, col_btn = st.columns([4, 1])
                with col_chip:
                    st.markdown(
                        f'<div style="background:#0f1520;border:1px solid #1e3a5f;border-radius:10px;'
                        f'padding:0.4rem 0.75rem;margin-bottom:0.25rem">'
                        f'<div style="font-size:0.83rem;color:#cbd5e1">{info.get("emoji","🍽️")} {s}</div>'
                        f'<div style="font-size:0.71rem;color:#334155">{kcal_pad:.0f} kcal · {qtd_pad:.0f}{info["unidade"]}</div>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                with col_btn:
                    if st.button("➕", key=f"quick_{s}", help=f"Adicionar {s}"):
                        udata["alimentacao"][selected_str].append({
                            "alimento": s, "tipo": tipo_rapido,
                            "quantidade": qtd_pad, "unidade": info["unidade"],
                            "calorias": kcal_pad,
                            "proteina": mac_pad.get("proteina", 0),
                            "carbo":    mac_pad.get("carbo", 0),
                            "gordura":  mac_pad.get("gordura", 0),
                            "foto": "", "comentario": "",
                        })
                        save()
                        st.toast(f"✅ {s} adicionado! {kcal_pad:.0f} kcal")
                        st.rerun()
            st.markdown("---")

        # ── Favoritos rápidos ────────────────────────────────────────────
        if udata["favoritos"]:
            st.markdown('<div style="font-size:0.82rem;font-weight:700;color:#64748b;margin-bottom:0.5rem">⭐ Favoritos</div>', unsafe_allow_html=True)
            for fi, fav in enumerate(udata["favoritos"]):
                fa1, fa2 = st.columns([3, 1])
                with fa1:
                    st.markdown(
                        '<div style="background:#0f1520;border:1px solid #1e2a3a;border-radius:10px;padding:0.45rem 0.75rem;margin-bottom:0.25rem">'
                        f'<div style="font-size:0.83rem;color:#cbd5e1;font-weight:500">{fav["alimento"]}</div>'
                        f'<div style="font-size:0.72rem;color:#334155">{fav.get("tipo","")} · {fav["calorias"]:.0f} kcal</div>'
                        '</div>',
                        unsafe_allow_html=True
                    )
                with fa2:
        ...
        if kcal_auto > 0 and macros_auto:
            st.markdown(
                '<div style="background:#080b10;border-radius:8px;padding:0.5rem 0.75rem;margin:0.3rem 0;'
                'display:flex;gap:1.5rem;font-size:0.75rem;color:#475569;border:1px solid #1e2a3a">'
                f'<span>🥩 P: <b style="color:#22d3ee">{macros_auto.get("proteina",0):.1f}g</b></span>'
                f'<span>🍚 C: <b style="color:#fbbf24">{macros_auto.get("carbo",0):.1f}g</b></span>'
                f'<span>🥑 G: <b style="color:#818cf8">{macros_auto.get("gordura",0):.1f}g</b></span>'
                '</div>',
                unsafe_allow_html=True
            )
            ...
            if st.button("✓ Adicionar", use_container_width=True):
                if alimento.strip():
                    ...
                    udata["alimentacao"][selected_str].append({
                        ...
                        "calorias": calorias,
                        "proteina": ma.get("proteina", 0),
                        "carbo":    ma.get("carbo", 0),
                        "gordura":  ma.get("gordura", 0),
    ...
    with col_view:
        ...
        # Cabeçalho da coluna — apenas texto, sem divs complexos
        cv1, cv2 = st.columns([2, 1])
        with cv1:
            st.markdown(
                f'<div style="font-family:Syne,sans-serif;font-size:0.95rem;font-weight:700;color:#e2e8f0;padding:0.3rem 0">'
                f'🗓️ {selected_date.strftime("%d/%m/%Y")}</div>',
                unsafe_allow_html=True
            )
        with cv2:
            st.markdown(
                f'<div style="font-family:Syne,sans-serif;font-size:1.1rem;font-weight:800;color:#22d3ee;text-align:right;padding:0.3rem 0">'
                f'{kcal_dia:.0f} kcal</div>',
                unsafe_allow_html=True
            )

        # ── Cada tipo de refeição ─────────────────────────────────────────
        # REGRA ABSOLUTA: st.markdown com HTML nunca contém st.button.
        # O header e os food-rows são HTML puro (sem widgets).
        # O botão 🗑️ fica em st.columns separado, APÓS o HTML.
        for tipo in TIPOS_REFEICAO:
            ...
            # Header: usa colunas nativas (sem div wrapper)
            th1, th2 = st.columns([3, 1])
            with th1:
                st.markdown(
                    f'<div class="meal-section-label">{tipo}</div>',
                    unsafe_allow_html=True
                )
            with th2:
                if itens:
                    st.markdown(
                        f'<div class="meal-section-kcal" style="text-align:right;padding:0.5rem 0 0.3rem">{tot_t:.0f} kcal</div>',
                        unsafe_allow_html=True
                    )

            if not itens:
                st.markdown(
                    '<div style="font-size:0.78rem;color:#1e3a5f;font-style:italic;padding:0.1rem 0 0.5rem 0.2rem">Nenhum registro</div>',
                    unsafe_allow_html=True
                )
            else:
                for orig_i, r in itens:
                    # food-row HTML: autônomo, sem widgets dentro
                    ali_nome  = r.get("alimento", "")
                    ali_qtd   = r.get("quantidade", 0)
                    ali_uni   = r.get("unidade", "")
                    ali_prot  = r.get("proteina", 0)
                    ali_kcal  = r.get("calorias", 0)
                    ali_com   = r.get("comentario", "")
                    prot_txt  = f" · P:{ali_prot:.0f}g" if ali_prot else ""
                    com_inner = (
                        f'<div style="font-size:0.7rem;color:#475569;font-style:italic;margin-top:0.15rem">💬 {ali_com}</div>'
                        if ali_com else ""
                    )
                    ...
                    with fr1:
                        st.markdown(
                            '<div class="food-row">'
                            '<div>'
                            f'<div class="food-row-name">{ali_nome}</div>'
                            f'<div class="food-row-qty">{ali_qtd} {ali_uni}{prot_txt}</div>'
                            f'{com_inner}'
                            '</div>'
                            f'<div class="food-row-kcal">{ali_kcal:.0f}</div>'
                            '</div>',
                            unsafe_allow_html=True
                        )
                    ...
                    with fr2:
                        if st.button("🗑️", key=f"df_{orig_i}_{selected_str}_{tipo[:3]}"):
        ...
        if refeicoes:
            ...
            st.markdown(
                '<div style="display:flex;justify-content:space-between;padding:0.7rem 1rem;'
                'background:#0f1520;border-radius:11px;border:1px solid #1e2a3a;margin-top:0.6rem">'
                '<span style="color:#334155;font-size:0.83rem">Saldo do dia</span>'
                f'<span style="font-family:Syne,sans-serif;font-weight:700;color:{cor_s};font-size:0.9rem">{saldo_txt} da meta</span>'
                '</div>',
                unsafe_allow_html=True
            )
...
with tab2:
    ...
    if st.session_state.editar_treino:
        ...
        st.markdown(
            '<div style="background:#0f1520;border:1.5px solid #22d3ee;border-radius:16px;'
    ...
    with col_tl:
        ...
        for i in range(30):
            ...
            with tc:
                st.markdown(
                    f'<div style="background:{bg};border-radius:11px;padding:0.65rem 1rem;'
                    f'margin-bottom:0.3rem;border-left:3px solid {bord};'
                    f'display:flex;justify-content:space-between;align-items:center">'
                    f'<div>'
                    f'<div style="font-size:0.72rem;color:#1e2a3a">{dl} · {sem}</div>'
                    f'<div style="color:#64748b;font-weight:500;font-size:0.84rem">{desc_t}</div>'
                    f'</div>'
                    f'<span style="font-weight:600;font-size:0.83rem;color:{cor_lbl}">{lbl}</span>'
                    f'</div>',
                    unsafe_allow_html=True
                )
            with te:
...
with tab3:
    ...
    # ── Objetivo com progresso de peso ───────────────────────────────────
    obj_now_h = udata.get("objetivo", "Manutencao")
    perf_h    = udata.get("perfil", {})
    if "Perda" in obj_now_h and perf_h.get("peso"):
        delta_map = {"Perda saudavel (-0,5kg/sem)": -0.5, "Perda agressiva (-1kg/sem)": -1.0}
        kg_semana = abs(delta_map.get(obj_now_h, -0.5))
        dias_com  = len([d for d, v in udata["alimentacao"].items() if v])
        kg_perdido = (dias_com / 7) * kg_semana
        meta_kg_total = kg_semana * 4
        prog_pct = min(kg_perdido / meta_kg_total * 100, 100) if meta_kg_total > 0 else 0
        cor_obj_p = "#34d399"
        st.markdown(
            '<div style="background:#0a1e0a;border:1px solid #166534;border-radius:14px;padding:1rem 1.2rem;margin-bottom:1rem">'
            '<div style="display:flex;justify-content:space-between;align-items:flex-start">'
            '<div>'
            f'<div style="font-family:Syne,sans-serif;font-size:0.9rem;font-weight:700;color:#34d399">🎯 {obj_now_h}</div>'
            f'<div style="font-size:0.78rem;color:#475569;margin-top:0.2rem">Meta: -{meta_kg_total:.1f} kg em 4 semanas</div>'
            '</div>'
            f'<div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;color:#34d399">-{kg_perdido:.1f} kg</div>'
            '</div>'
            '<div class="obj-prog-wrap">'
            '<div class="obj-prog-bg">'
            f'<div class="obj-prog-fill" style="width:{prog_pct:.0f}%;background:linear-gradient(90deg,#22d3ee,#34d399)"></div>'
            '</div>'
            '</div>'
            f'<div style="font-size:0.73rem;color:#334155">Projecao: {28 / kg_semana * (kg_semana - kg_perdido / 4):.0f} dias restantes nesse ritmo</div>'
            '</div>',
            unsafe_allow_html=True
        )
    elif obj_now_h == "Ganho muscular":
        treinos_tot = sum(1 for t in udata["treinos"].values() if t.get("concluido"))
        st.markdown(
            '<div style="background:#1a0c2a;border:1px solid #818cf855;border-radius:14px;padding:1rem 1.2rem;margin-bottom:1rem">'
            f'<div style="font-family:Syne,sans-serif;font-size:0.9rem;font-weight:700;color:#818cf8">💪 Ganho Muscular</div>'
            f'<div style="font-size:0.78rem;color:#475569;margin-top:0.2rem">{treinos_tot} treinos concluidos · Superavit +300 kcal/dia</div>'
            '</div>',
            unsafe_allow_html=True
        )

    # ── Nível e pontos ────────────────────────────────────────────────────
    pts_h = calcular_pontos(udata)
    niv_h, niv_prox_h = calcular_nivel(pts_h)
    pts_prox = niv_prox_h["min_pts"] if niv_prox_h else pts_h
    pct_niv  = min((pts_h - niv_h["min_pts"]) / max(pts_prox - niv_h["min_pts"], 1) * 100, 100)
    st.markdown(
        '<div style="background:#0f1520;border:1px solid #1e2a3a;border-radius:14px;padding:1rem 1.2rem;margin-bottom:1rem">'
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem">'
        f'<div style="font-family:Syne,sans-serif;font-size:0.95rem;font-weight:700;color:#c4b5fd">{niv_h["emoji"]} Nivel: {niv_h["nome"]}</div>'
        f'<div style="font-family:Syne,sans-serif;font-size:0.85rem;font-weight:700;color:#818cf8">{pts_h} pts</div>'
        '</div>'
        '<div class="macro-bg">'
        f'<div class="macro-fill" style="width:{pct_niv:.0f}%;background:linear-gradient(90deg,#818cf8,#c4b5fd)"></div>'
        '</div>'
        f'<div style="font-size:0.73rem;color:#334155;margin-top:0.3rem">'
        f'{f"Proximo nivel: {niv_prox_h[chr(110)+chr(111)+chr(109)+chr(101)]} em {pts_prox - pts_h} pts" if niv_prox_h else "Nivel maximo atingido! 👑"}'
        '</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    if insights:
        ...
        for ins in insights:
            st.markdown(
                ...
                f'<div class="insight-ico">{ins["emoji"]}</div>'
                f'<div class="insight-txt">{ins["txt"]}</div>'
                '</div>',
                unsafe_allow_html=True
            )
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    # Media semanal destacada
    media_sem_h = round(sum(kcal_30[-7:]) / max(len([k for k in kcal_30[-7:] if k > 0]), 1), 0)
    diff_media  = media_sem_h - META
    cor_media   = "#34d399" if abs(diff_media) < META * 0.1 else ("#fb7185" if diff_media > 0 else "#fbbf24")
    st.markdown(
        '<div style="display:flex;gap:1rem;margin:0.5rem 0 1rem;flex-wrap:wrap">'
        '<div style="flex:1;background:#0f1520;border:1px solid #1e2a3a;border-radius:12px;padding:0.75rem 1rem;text-align:center">'
        f'<div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;color:{cor_media}">{media_sem_h:.0f}</div>'
        '<div style="font-size:0.7rem;color:#475569;margin-top:0.2rem">Media kcal (7d)</div>'
        '</div>'
        '<div style="flex:1;background:#0f1520;border:1px solid #1e2a3a;border-radius:12px;padding:0.75rem 1rem;text-align:center">'
        f'<div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;color:#fbbf24">{sum(1 for k in kcal_30[-7:] if k > 0)}</div>'
        '<div style="font-size:0.7rem;color:#475569;margin-top:0.2rem">Dias registrados (7d)</div>'
        '</div>'
        '<div style="flex:1;background:#0f1520;border:1px solid #1e2a3a;border-radius:12px;padding:0.75rem 1rem;text-align:center">'
        f'<div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;color:#818cf8">{sum(1 for d in dias_30[-7:] if udata["treinos"].get(d,{}).get("concluido"))}</div>'
        '<div style="font-size:0.7rem;color:#475569;margin-top:0.2rem">Treinos (7d)</div>'
        '</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    if rows:
        # Botão repetir dia no histórico
        datas_disp = [r["Data"] for r in rows]
        if datas_disp:
            rep_col1, rep_col2 = st.columns([3, 1])
            with rep_col1:
                dia_rep = st.selectbox("Repetir dia no diario de hoje:", datas_disp, key="rep_dia_sel")
            with rep_col2:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("📋 Repetir", key="btn_rep_hist"):
                    import copy
                    if dia_rep and not udata["alimentacao"].get(today_str):
                        udata["alimentacao"][today_str] = copy.deepcopy(udata["alimentacao"].get(dia_rep, []))
                        save()
                        st.success(f"Dia {dia_rep} copiado para hoje!")
                        st.rerun()
                    else:
                        st.warning("Hoje ja tem registros ou dia invalido.")
    ...
    for ci, c in enumerate(CONQUISTAS):
        ...
        bg_c  = "linear-gradient(135deg,#1a1000,#2a1a00)" if desbloqueada else "#0f1520"
        bord_c = "#fbbf2444" if desbloqueada else "#1e2a3a"
        cor_c  = "#fbbf24" if desbloqueada else "#1e2a3a"
        txt_c  = "#78716c" if desbloqueada else "#1e2a3a"
        emo_c  = c["emoji"] if desbloqueada else "🔒"
        pts_c  = f'+{c["pts"]} pts' if desbloqueada else f'{c["pts"]} pts'
        with c_cols[ci % 2]:
            st.markdown(
                f'<div style="background:{bg_c};border:1px solid {bord_c};border-radius:12px;'
                f'padding:0.75rem 0.9rem;display:flex;gap:0.6rem;align-items:center;margin-bottom:0.5rem">'
                f'<div style="font-size:1.5rem;flex-shrink:0">{emo_c}</div>'
                f'<div style="flex:1">'
                f'<div style="font-family:Syne,sans-serif;font-size:0.83rem;font-weight:700;color:{cor_c}">{c["nome"]}</div>'
                f'<div style="font-size:0.74rem;color:{txt_c};margin-top:0.1rem">{c["desc"]}</div>'
                f'</div>'
                f'<div style="font-family:Syne,sans-serif;font-size:0.75rem;font-weight:700;color:{cor_c}">{pts_c}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
    ...
    st.download_button(
        ...
        file_name=f"nutriflow_{uname}.json",
...
with tab4:
    ...
    with sc1:
        ...
        for u, ud in todos.items():
            ...
            rank.append({
                "u": u, "nome": ud["nome"],
                "pct": k_u / m_u * 100 if m_u > 0 else 0,
        ...
        for pos, r in enumerate(rank):
            ...
            bord_r = "border:1.5px solid #0e7490;" if r["u"] == uname else "border:1px solid #1e2a3a;"
            pct2   = min(r["pct"], 100)
            ini_r  = r["nome"][0].upper()
            is_me  = r["u"] == uname
            st.markdown(
                f'<div class="rank-item" style="{bord_r}">'
                f'<div style="font-family:Syne,sans-serif;font-weight:800;font-size:1rem;width:28px;text-align:center">{emo_r}</div>'
                f'<div style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#22d3ee,#818cf8);'
                f'display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:700;'
                f'font-size:0.82rem;color:#080b10;flex-shrink:0">{ini_r}</div>'
                f'<div style="flex:1">'
                f'<div style="font-size:0.87rem;font-weight:600;color:#e2e8f0">{r["nome"]} {"👑" if is_me else ""}</div>'
                f'<div style="font-size:0.73rem;color:#334155">{r["kcal"]:.0f}/{r["meta"]} kcal · 🔥{r["streak"]}d</div>'
                f'</div>'
                f'<div style="text-align:right">'
                f'<div style="font-family:Syne,sans-serif;font-size:0.85rem;font-weight:700;color:#22d3ee">{pct2:.0f}%</div>'
                f'<div style="width:55px;background:#131c2a;border-radius:999px;height:4px;margin-top:3px">'
                f'<div style="width:{pct2:.0f}%;background:#22d3ee;height:4px;border-radius:999px"></div>'
                f'</div>'
                f'</div>'
                f'</div>',
                unsafe_allow_html=True
        ...
        # ── Ranking semanal ────────────────────────────────────────────────
        st.markdown("<br>")
        st.markdown("**📅 Ranking semanal (kcal acumuladas)**")
        rank_sem = []
        dias_7 = [(date.today() - timedelta(days=i)).isoformat() for i in range(7)]
        for u, ud in todos.items():
            k7 = sum(total_kcal_dia(ud, d) for d in dias_7)
            m7 = ud.get("meta_calorias", 2000) * 7
            rank_sem.append({"u": u, "nome": ud["nome"], "k7": k7, "m7": m7, "pct7": k7/m7*100 if m7>0 else 0})
        rank_sem.sort(key=lambda x: x["pct7"], reverse=True)

        for pos, r in enumerate(rank_sem):
            emo_r  = ["🥇","🥈","🥉"][pos] if pos < 3 else f"{pos+1}."
            bord_r = "border:1.5px solid #0e7490;" if r["u"] == uname else "border:1px solid #1e2a3a;"
            pct7   = min(r["pct7"], 100)
            st.markdown(
                f'<div class="rank-item" style="{bord_r}">'
                f'<span style="font-family:Syne,sans-serif;font-weight:800;font-size:0.95rem;width:24px">{emo_r}</span>'
                f'<span style="flex:1;font-size:0.87rem;color:#e2e8f0">{r["nome"]}</span>'
                f'<span style="font-size:0.73rem;color:#334155;margin-right:0.5rem">{r["k7"]:.0f} kcal</span>'
                f'<div style="width:55px;background:#131c2a;border-radius:999px;height:4px">'
                f'<div style="width:{pct7:.0f}%;background:#818cf8;height:4px;border-radius:999px"></div>'
                f'</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        ...
        st.markdown("<br>")
        st.markdown("**💪 Desafio de treinos (7 dias)**")
        rank_t = sorted(
            [{"u": u, "nome": ud["nome"],
        ...
        for pos, r in enumerate(rank_t):
            ...
            st.markdown(
                ...
                f'<span style="font-family:Syne,sans-serif;font-weight:800;font-size:0.95rem;width:24px">{emo_r}</span>'
                f'<span style="flex:1;font-size:0.87rem;color:#e2e8f0">{r["nome"]}</span>'
                f'<div style="flex:1;margin:0 0.5rem">'
                f'<div style="background:#131c2a;border-radius:999px;height:5px">'
                f'<div style="width:{b}%;background:#818cf8;height:5px;border-radius:999px"></div>'
                f'</div>'
                f'</div>'
                f'<span style="font-family:Syne,sans-serif;font-size:0.85rem;font-weight:700;color:#818cf8">{r["t7"]}/7</span>'
                f'</div>',
                unsafe_allow_html=True
            )

    with sc2:
        # ── Botão "Bati minha meta!" ──────────────────────────────────────
        kcal_user_hoje = total_kcal_dia(udata, today_str)
        if kcal_user_hoje >= META * 0.95 and kcal_user_hoje > 0:
            if st.button("🔥 Bati minha meta hoje! Compartilhar", use_container_width=True):
                chave_evento = f"meta_{uname}_{today_str}"
                udata["reacoes"].setdefault(chave_evento, {"tipo": "meta", "reacoes": {}, "usuario": uname, "nome": udata["nome"], "data": today_str, "kcal": kcal_user_hoje})
                save()
                st.toast("🔥 Compartilhado no feed!")
                st.rerun()
        else:
            falt_meta = META - kcal_user_hoje
            st.markdown(
                f'<div style="background:#0f1520;border:1px solid #1e2a3a;border-radius:12px;'
                f'padding:0.65rem 1rem;margin-bottom:0.5rem;font-size:0.82rem;color:#475569">'
                f'Faltam <b style="color:#22d3ee">{falt_meta:.0f} kcal</b> para compartilhar sua conquista!'
                f'</div>',
                unsafe_allow_html=True
            )
        ...
        # Eventos de "bati a meta" compartilhados
        for u, ud in todos.items():
            for ev_key, ev in ud.get("reacoes", {}).items():
                if ev.get("data") == today_str and ev.get("tipo") == "meta":
                    feed.append({
                        "nm": ev.get("nome", u), "u": u, "h": "hoje",
                        "txt": f"🎯 bateu a meta! {ev.get('kcal',0):.0f} kcal consumidas",
                        "ev_key": ev_key, "reacoes": ev.get("reacoes", {}),
                    })
        ...
        for u, ud in todos.items():
            ...
            if t_h.get("concluido") and t_h.get("descricao"):
                feed.append({"nm": nm, "u": u, "h": "hoje", "txt": f"💪 concluiu {t_h['descricao']}", "ev_key": None, "reacoes": {}})
            s_ = calcular_streak(ud)
            if s_ >= 3:
                feed.append({"nm": nm, "u": u, "h": "hoje", "txt": f"🔥 {s_} dias consecutivos de treino!", "ev_key": None, "reacoes": {}})
            ont = (date.today() - timedelta(days=1)).isoformat()
            t_o = ud["treinos"].get(ont, {})
            if t_o.get("concluido") and t_o.get("descricao"):
                feed.append({"nm": nm, "u": u, "h": "ontem", "txt": f"🏋️ treinou {t_o['descricao']} ontem", "ev_key": None, "reacoes": {}})

        if not feed:
            st.markdown(
                '<div style="color:#1e293b;text-align:center;padding:2rem;font-size:0.85rem">'
                'Sem atividade ainda. Comece a registrar!'
                '</div>',
                unsafe_allow_html=True
            )
        else:
            random.shuffle(feed)
            for fi_idx, fi in enumerate(feed[:12]):
                ini_f  = fi["nm"][0].upper()
                bord_f = "border:1.5px solid #0e7490;" if fi["u"] == uname else "border:1px solid #1e2a3a;"
                is_me_f = fi["u"] == uname
                st.markdown(
                    f'<div class="feed-item" style="{bord_f}">'
                    f'<div class="feed-av">{ini_f}</div>'
                    f'<div style="flex:1">'
                    f'<div style="font-weight:600;font-size:0.85rem;color:#e2e8f0">{fi["nm"]} {"(voce)" if is_me_f else ""}</div>'
                    f'<div style="font-size:0.8rem;color:#475569;margin-top:0.1rem">{fi["txt"]}</div>'
                    f'<div style="font-size:0.7rem;color:#1e2a3a;margin-top:0.15rem">{fi["h"]}</div>'
                    f'</div>'
                    f'</div>',
                    unsafe_allow_html=True
                )
                # Reações — apenas para eventos com chave
                if fi.get("ev_key"):
                    reacoes_ev = fi.get("reacoes", {})
                    tot_fire   = reacoes_ev.get("🔥", 0)
                    tot_like   = reacoes_ev.get("👍", 0)
                    r1, r2, _ = st.columns([1, 1, 5])
                    with r1:
                        if st.button(f"🔥 {tot_fire}", key=f"reac_fire_{fi_idx}_{fi['u']}"):
                            ev_ref = None
                            for u2, ud2 in todos.items():
                                if fi["ev_key"] in ud2.get("reacoes", {}):
                                    ev_ref = ud2["reacoes"][fi["ev_key"]]
                                    break
                            if ev_ref is not None:
                                ev_ref["reacoes"]["🔥"] = ev_ref["reacoes"].get("🔥", 0) + 1
                                save()
                                st.rerun()
                    with r2:
                        if st.button(f"👍 {tot_like}", key=f"reac_like_{fi_idx}_{fi['u']}"):
                            ev_ref = None
                            for u2, ud2 in todos.items():
                                if fi["ev_key"] in ud2.get("reacoes", {}):
                                    ev_ref = ud2["reacoes"][fi["ev_key"]]
        ...
        # ── Metas semanais pessoais ───────────────────────────────────────
        st.markdown("<br>")
        st.markdown("**🎯 Minha semana**")
        ...
        for lbl, val, cor_m in [
            ...
            st.markdown(
                ...
                f'<span>{lbl}</span>'
                f'<span style="color:{cor_m};font-family:Syne,sans-serif;font-weight:700">{val}/7</span>'
                '</div>'
                '<div style="background:#1e2a3a;border-radius:999px;height:7px">'
                f'<div style="width:{pm:.0f}%;background:{cor_m};height:7px;border-radius:999px"></div>'
                '</div>'
                '</div>',
                unsafe_allow_html=True
            )
        ...
        if outros:
            st.markdown("<br>")
            st.markdown("**⚔️ Comparar com amigo**")
            amigo = st.selectbox("Escolher usuario:", outros, key="cmp_amigo")
            if amigo:
                ud_amigo  = todos[amigo]
                k_amigo   = total_kcal_dia(ud_amigo, today_str)
                k_eu      = total_kcal_dia(udata, today_str)
                m_amigo   = ud_amigo.get("meta_calorias", 2000)
                pct_eu    = min(k_eu / META * 100, 100) if META > 0 else 0
                pct_amigo = min(k_amigo / m_amigo * 100, 100) if m_amigo > 0 else 0
                st.markdown(
                    '<div style="background:#0f1520;border:1px solid #1e2a3a;border-radius:14px;padding:1rem 1.2rem">'
                    '<div style="display:flex;justify-content:space-between;margin-bottom:0.75rem">'
                    f'<div style="font-family:Syne,sans-serif;font-size:0.9rem;font-weight:700;color:#22d3ee">👑 {udata["nome"]}</div>'
                    f'<div style="font-family:Syne,sans-serif;font-size:0.9rem;font-weight:700;color:#818cf8">{ud_amigo["nome"]}</div>'
                    '</div>'
                    '<div style="display:flex;justify-content:space-between;font-size:0.78rem;color:#475569;margin-bottom:0.4rem">'
                    f'<span>{k_eu:.0f} / {META} kcal</span>'
                    f'<span>{k_amigo:.0f} / {m_amigo} kcal</span>'
                    '</div>'
                    '<div style="display:flex;gap:0.4rem;align-items:center">'
                    '<div style="flex:1;background:#1e2a3a;border-radius:999px;height:8px;overflow:hidden">'
                    f'<div style="width:{pct_eu:.0f}%;background:#22d3ee;height:8px;border-radius:999px;float:right"></div>'
                    '</div>'
                    '<div style="font-family:Syne,sans-serif;font-size:0.75rem;font-weight:700;color:#475569;padding:0 0.3rem">VS</div>'
                    '<div style="flex:1;background:#1e2a3a;border-radius:999px;height:8px;overflow:hidden">'
                    f'<div style="width:{pct_amigo:.0f}%;background:#818cf8;height:8px;border-radius:999px"></div>'
                    '</div>'
                    '</div>'
                    '</div>',
                    unsafe_allow_html=True
...
with tab5:
    ...
    with pp1:
        ...
        st.markdown(
            '<div style="background:#080b10;border-radius:12px;padding:1rem;text-align:center;margin:0.8rem 0;border:1px solid #1e2a3a">'
            '<div style="display:flex;justify-content:space-around">'
            '<div>'
            '<div style="font-size:0.68rem;color:#334155;text-transform:uppercase">TMB</div>'
            f'<div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;color:#818cf8">{tmb_:.0f}</div>'
            '</div>'
            '<div>'
            '<div style="font-size:0.68rem;color:#334155;text-transform:uppercase">TDEE</div>'
            f'<div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;color:#22d3ee">{tde_:.0f}</div>'
            '</div>'
            '<div>'
            '<div style="font-size:0.68rem;color:#334155;text-transform:uppercase">Nova Meta</div>'
            f'<div style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;color:#34d399">{nm_}</div>'
            '</div>'
            '</div>'
            f'{proj_html}'
            '</div>',
    ...
    with pp2:
        ...
        pts_perfil   = calcular_pontos(udata)
        niv_perfil, _ = calcular_nivel(pts_perfil)

        for lbl, val, cv in [
            ("Usuario",    f"@{uname}",                            "#64748b"),
            ("Nivel",      f"{niv_perfil['emoji']} {niv_perfil['nome']} · {pts_perfil} pts", "#c4b5fd"),
            ...
            st.markdown(
                ...
                f'<span style="color:{cv};font-weight:500">{val}</span>'
                '</div>',
                unsafe_allow_html=True
            )
        ...
        if udata["favoritos"]:
            ...
            for fi, fav in enumerate(udata["favoritos"]):
                ...
                with fg1:
                    st.markdown(
                        f'<div style="font-size:0.8rem;color:#475569;padding:0.15rem 0">{fav["alimento"][:30]}</div>',
                        unsafe_allow_html=True
                    )
                with fg2:
...
# ── RODAPÉ ────────────────────────────────────────────────────────────────────
st.markdown(
    f'<div style="text-align:center;color:#1e293b;font-size:0.7rem;margin-top:2.5rem;'
    f'padding:0.8rem 0;border-top:1px solid #1e2a3a">'
    f'⚡ NutriFlow v5 · @{uname} · <code style="color:#1e293b">{DATA_FILE}</code>'
    f'</div>',
    unsafe_allow_html=True
)